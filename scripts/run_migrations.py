...
def get_migration_files(migrations_dir="migrations"):
    ...
    # Look for migrations directory in parent directory if not found in current directory
    migrations_path = Path(migrations_dir)
    if not migrations_path.exists():
        # Try parent directory
        parent_migrations_path = Path("..") / migrations_dir
        if parent_migrations_path.exists():
            migrations_path = parent_migrations_path
        else:
...
def show_migration_status(use_docker=False):
    """Show the status of all migrations"""
    db_url = get_database_url(use_docker)
    print(f"Migration status for database: {db_url}")
...
def main():
    ...
    parser.add_argument(
        "--docker", action="store_true", help="Use Docker database configuration"
    )
    parser.add_argument(
    ...
    if args.status:
        show_migration_status(use_docker=args.docker)
    else:
