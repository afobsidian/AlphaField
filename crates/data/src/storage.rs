...
impl HistoricalDataStorage {
    ...
    fn get_file_path(&self, symbol: &str, interval: &str) -> PathBuf {
        ...
        self.base_dir.join(format!("{}_{}.json", safe_symbol, interval))
    }
    ...
    pub fn save_bars(&self, symbol: &str, interval: &str, bars: &[Bar]) -> Result<()> {
        ...
        serde_json::to_writer(file, bars).map_err(|e| QuantError::Parse(format!("Serialization error: {}", e)))?;
        Ok(())
    }
    ...
    pub fn load_bars(&self, symbol: &str, interval: &str) -> Result<Vec<Bar>> {
        let path = self.get_file_path(symbol, interval);
        if !path.exists() {
            return Err(QuantError::NotFound(format!("Data file not found: {:?}", path)));
        }
        let file = fs::File::open(path).map_err(QuantError::Io)?;
        let bars: Vec<Bar> = serde_json::from_reader(file).map_err(|e| QuantError::Parse(format!("Deserialization error: {}", e)))?;
        Ok(bars)
    }
