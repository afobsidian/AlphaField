...
const STRATEGY_UI_OVERRIDES = {
    ...
    DivergenceStrategy: {
        ...
        description:
            "Price-sentiment divergence detection for reversal signals",
    },
    ...
    `;
        ...
        categoryStrategies.forEach((s) => {
            ...
            input.addEventListener("change", (e) => {
                ...
                document
                    .querySelectorAll(".strategy-compact-item")
                    .forEach((i) => {
...
function initAnalysisModeToggle() {
    document
        .querySelectorAll('input[name="analysis-mode"]')
        .forEach((radio) => {
            radio.addEventListener("change", (e) => {
                ...
                document
                    .getElementById("comprehensive-results")
                    .classList.add("hidden");
                document.getElementById("wfa-results").classList.add("hidden");
                document
                    .getElementById("sensitivity-results")
                    .classList.add("hidden");
                document
                    .getElementById("optimize-results-placeholder")
...
function updateDeploySummary() {
    ...
    document.getElementById("deploy-symbol").textContent =
        AppState.symbol || "—";
...
async function loadSentiment() {
    ...
    try {
        ...
        if (data.success && data.data && data.data.length > 0) {
            ...
            document.getElementById("gauge-large-fill").style.width =
                `${value}%`;
            ...
            // 7-Day SMA
            const sma7 =
                stats.sma_7 !== undefined
                    ? stats.sma_7
                    : calculateSMA(history, 7);
            ...
            document.getElementById("sent-dominance").textContent =
                dominanceText;
...
function updateStatPill(id, value, suffix, colorize = false) {
    ...
    const formatted =
        Math.abs(value) < 10 ? value.toFixed(1) : value.toFixed(0);
    const sign = value > 0 ? "+" : "";
...
function updatePortfolioDisplay(data) {
    ...
    document.getElementById("live-cash").textContent = formatCurrency(
        data.cash,
    );
    document.getElementById("live-positions-value").textContent =
...
async function runComprehensiveWorkflow() {
    ...
    try {
        const include3D = document.getElementById(
            "include-3d-sensitivity",
        ).checked;
    ...
    const data = [
        {
            ...
            text: sweepResults.map(
                (r) =>
                    `Score: ${r.score.toFixed(2)}<br>Trades: ${r.total_trades}`,
            ),
        ...
        const data = [
            {
                ...
                y: windows.map(
                    (w) =>
                        ((w.test_metrics && w.test_metrics.total_return) || 0) *
                        100,
                ),
                name: "OOS Return",
                type: "bar",
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document
        .querySelectorAll('input[name="asset-category"]')
        .forEach((input) => {
...
function renderRsiChart(indicator) {
    const timestamps = indicator.values.map(
        (v) => new Date(v.timestamp * 1000),
    );
    const values = indicator.values.map((v) => v.value);
...
function updatePendingOrdersTable(orders) {
    ...
    document.getElementById("queue-total-qty").textContent =
        totalQty.toFixed(2);
    ...
    const bracketOrder = {
        entry: {
            ...
            price:
                entryType === "Limit"
                    ? parseFloat(
                          document.getElementById("bracket-entry-price").value,
                      )
                    : null,
            status: "New",
            timestamp: new Date().toISOString(),
...
async function createLimitChaseOrder(event) {
    ...
    const chaseOrder = {
        ...
        is_percentage:
            document.getElementById("chase-type").value === "percentage",
        max_adjustments: parseInt(
            document.getElementById("chase-max-adj").value,
        ),
    };
...
async function partialTakeProfit(event) {
    ...
    const tpRequest = {
        ...
        remaining_qty: parseFloat(
            document.getElementById("tp-remaining").value,
        ),
    };
...
async function cancelBracketOrder(bracketId) {
    try {
        const response = await fetch(
            `/api/orders/bracket/${bracketId}/cancel`,
            {
                method: "POST",
            },
        );
        if (response.ok) {
            logMessage(
                `Bracket order ${bracketId.substring(0, 8)} canceled`,
                "info",
            );
            loadActiveAdvancedOrders();
        }
...
async function cancelIcebergOrder(icebergId) {
    try {
        const response = await fetch(
            `/api/orders/iceberg/${icebergId}/cancel`,
            {
                method: "POST",
            },
        );
        if (response.ok) {
            logMessage(
                `Iceberg order ${icebergId.substring(0, 8)} canceled`,
                "info",
            );
            loadActiveAdvancedOrders();
        }
...
async function cancelLimitChaseOrder(chaseId) {
    try {
        const response = await fetch(
            `/api/orders/limit-chase/${chaseId}/cancel`,
            {
                method: "POST",
            },
        );
        if (response.ok) {
            logMessage(
                `Limit chase ${chaseId.substring(0, 8)} canceled`,
                "info",
            );
            loadActiveAdvancedOrders();
        }
