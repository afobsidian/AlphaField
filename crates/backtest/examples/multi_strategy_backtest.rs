...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let strategies: Vec<(
        &str,
        Box<dyn Fn() -> Box<dyn alphafield_core::Strategy + Send + Sync>>,
    )> = vec![
        (
            "Golden Cross (20/50)",
            Box::new(|| Box::new(GoldenCrossStrategy::new(20, 50))),
        ),
        (
            "Golden Cross (10/30)",
            Box::new(|| Box::new(GoldenCrossStrategy::new(10, 30))),
        ),
        (
            "RSI (14, 30/70)",
        ...
        (
            "Momentum (50, 12/26/9)",
            Box::new(|| Box::new(MomentumStrategy::new(50, 12, 26, 9))),
        ),
    ];
    ...
    for (name, strategy_fn) in &strategies {
        ...
        let adapter = StrategyAdapter::new(strategy_fn(), symbol, 100_000.0);
    ...
    // 4. Summary
    if let Some((best_name, best_return, _, _)) =
    ...
    if let Some((worst_name, worst_return, _, _)) =
