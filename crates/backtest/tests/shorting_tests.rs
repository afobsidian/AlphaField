...
fn rejects_selling_more_than_owned() {
    ...
    p.update_from_fill("BTC", 1.0, 100.0, 0.0).expect("buy should succeed");

    // Attempt to sell 2.0 units -> should error with InsufficientPosition
    let res = p.update_from_fill("BTC", -2.0, 100.0, 0.0);
    match res {
        Err(BacktestError::InsufficientPosition { symbol, required, available }) => {
