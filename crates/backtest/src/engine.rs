...
use crate::strategy::{Strategy, OrderRequest, OrderSide, OrderType};
...
impl BacktestEngine {
    ...
    pub fn run(&mut self) -> Result<PerformanceMetrics> {
        ...
        let driver_symbol = self.data.keys().next().ok_or(crate::error::BacktestError::Data("No data loaded".to_string()))?.clone();
        ...
        for bar in bars {
            ...
            if let Some(strategy) = &mut self.strategy {
                ...
                for order in orders {
                    ...
                    self.portfolio.update_from_fill(&order.symbol, order.quantity, price, fee)?;
                }
            }
        }

        Ok(PerformanceMetrics::calculate(&self.portfolio.equity_history, 0.02)) // 2% risk free rate
    }
}
