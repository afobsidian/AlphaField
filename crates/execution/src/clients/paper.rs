...
impl ExecutionService for PaperTradingClient {
    async fn submit_order(&self, order: &Order) -> Result<String> {
        let mut orders = self.orders.lock().map_err(|_| QuantError::Io(std::io::Error::new(std::io::ErrorKind::Other, "Lock poisoned")))?;
    ...
    async fn cancel_order(&self, order_id: &str, _symbol: &str) -> Result<()> {
        ...
        if let Some(order) = orders.get_mut(order_id) {
            if order.status == OrderStatus::New || order.status == OrderStatus::PartiallyFilled {
                ...
                Err(QuantError::DataValidation(format!("Cannot cancel order in status {:?}", order.status)))
            }
        } else {
            Err(QuantError::NotFound(format!("Order {} not found", order_id)))
        }
    }

    async fn get_order(&self, order_id: &str, _symbol: &str) -> Result<Order> {
        let orders = self.orders.lock().map_err(|_| QuantError::Io(std::io::Error::new(std::io::ErrorKind::Other, "Lock poisoned")))?;
